...
4. For each PDF or HTML snapshot attachment found, the script resolves or calculates the item's Zotero Citation Key (e.g. `Author-Year-ZoteroKey` or organizational acronym). It downloads the attachment locally using Zotero's attachment dump API with the Citation Key as filename (`{citationKey}.pdf`). If the attachment is an HTML snapshot, it uses Microsoft Edge in headless mode to convert the HTML file to a print-ready PDF (`{citationKey}.pdf`). The script uploads the resulting PDF directly to NotebookLM using `nlm source add` with `--title {citationKey}`, avoiding duplicate uploads by checking existing notebook sources and local tracking JSON, assigns it to a collection/subcollection label, and immediately deletes all temporary local files.
5. Upon successful uploads, the script saves or merges the metadata into `zotero_uploaded_items.json` and `_zotero_uploaded_items.json` in the workspace root directory.
